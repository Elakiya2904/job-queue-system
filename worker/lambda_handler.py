...
# Set up DATABASE_URL before importing app
...
try:
    ...
    from app.models import Task, TaskAttempt, Worker
...
class LambdaWorkerHandler:
    ...
    def _register_worker(self):
        """Auto-register this Lambda worker in the database."""
        try:
            session = SessionLocal()
            worker = session.query(Worker).filter(Worker.id == self.worker_id).first()
            
            if not worker:
                logger.info(f"Registering Lambda worker: {self.worker_id}")
                worker = Worker(
                    id=self.worker_id,
                    name=f"Lambda Worker - {os.environ.get('AWS_LAMBDA_FUNCTION_NAME', 'unknown')}",
                    status="idle",
                    task_types="*",  # Can handle all task types
                ...
                logger.info(f"Worker {self.worker_id} already registered")
            
            session.close()
        except Exception as e:
            logger.error(f"Failed to register worker: {e}")
            # Don't fail the whole Lambda if registration fails; continue with processing
